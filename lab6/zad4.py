...
class BruteForceAttack:
    ...
    def _hamming_distance(self, bits1, bits2):
        ...
        for i in range(min_len):
            if bits1[i] != bits2[i]:
                distance += 1
            self.operations_count += 3  # Porównanie i inkrementacja
        return distance
    ...
    def attack(self):
        ...
        # Generujemy wszystkie możliwe 12-bitowe klucze (z wyłączeniem całkowicie zerowego)
        for key_bits in itertools.product([0,1], repeat=12):
            if sum(key_bits) == 0:
                continue  # Pomijamy klucz zerowy
            ...
            try:
                ...
                # Obliczamy odległość Hamminga między odzyskanym a wygenerowanym strumieniem
                distance = self._hamming_distance(self.recovered_key_bits, key_stream)
                self.operations_count += len(self.recovered_key_bits) * 5  # Szacunek dla generacji strumienia
                ...
                # Optymalizacja: jeśli znaleźliśmy idealne dopasowanie, przerywamy
                if distance == 0:
                    break
                    
            except ValueError:
                continue  # Pomijamy nieprawidłowe kombinacje (np. wszystkie zera w rejestrze)
